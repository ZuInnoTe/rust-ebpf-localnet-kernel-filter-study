...
* Use case implemented as a eBPF socket filter program : [sock-filter](sock-filter). This filters packets by certain users to certain IP addresses based on a certain request pattern and provides them to a user space program. Note: Socket filter cannot block/allow a certain packet to be processed. Its main purpose is demonstrating some auditing special case of the use case.
* Use case implemented as a eBPF uprobe libcall filter (e.g. for the shared OpenSSL Library):  [uprobe-libcall-filter](uprobe-libcall-filter). This allows getting unecrypted data by intercepting calls to the OpenSSL library. This allows you to get otherwise encrypted data (e.g. in network packets) without knowing the private key.  One can modify the call (ie the function is not called at all) to the OpenSSL library and return an error code instead or pretending that no data is available using [bpf_override_return](https://man7.org/linux/man-pages/man7/bpf-helpers.7.html). 
...
Find here some types of eBPF programs:
* Networking
  ...
  * socket: network packet filter. It has access to the data structure bfp_sock_addr (see [here for Linux Kernel 6.0](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/bpf.h?v=6.0#n6291)) Can rewrite the packet and the socket options. It has acceess to certain kernel internal eBPF functions (see [here for Linux Kernel 6.0](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/net/core/filter.c?h=v6.0#n7668)), Important: It does not act at the network level, but the socket level.
    * Filter: Filtering of packets received from sockets. This was also available in classic BPF. Note: You can NOT block/allow a packet at network level. You can only decide which packets are forwarded to a socket and which one are not. Essentially it is similar to what [tcpdump](https://www.tcpdump.org/) does. You can analyze all supported protocols of the Linux kernel.
    * Stream Parser: operates on a group of sockets that are added to a special BPF map. Processes messages send or received on those sockets.
...
There are more types and the possible types evolves in each Linux kernel version. See also a list provided [here](https://github.com/iovisor/bcc/blob/master/docs/kernel-versions.md#program-types) or you can check them in the kernel source (e.g. [here](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/tree/include/uapi/linux/bpf.h?h=v6.0#n922) for Linux 6.0).
