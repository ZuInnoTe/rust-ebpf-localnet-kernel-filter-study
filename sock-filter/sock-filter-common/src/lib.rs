...
/// Some config options for the eBPF program provided by the user space app
#[repr(C)]
#[derive(Clone, Copy)]
pub enum ConfigEbpf {
    AllowAll = 1,
...
/// This structure represents the filters configured via the user space application by which the eBPF program should filter the socket messages
#[repr(C)]
#[derive(Clone, Copy)]
